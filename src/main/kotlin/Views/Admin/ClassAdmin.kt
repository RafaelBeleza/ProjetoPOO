...
class Class {
    ...
    val tableView = TableView<ClassData>().apply {
        style = """
        ...
        columns += TableColumn<ClassData, Int>("ID").apply {
            cellValueFactory = PropertyValueFactory("id")
        }

        columns += TableColumn<ClassData, String>("Name").apply {
            cellValueFactory = PropertyValueFactory("name")
        }

        columns += TableColumn<ClassData, Int>("Grade ID").apply {
            cellValueFactory = PropertyValueFactory("gradeId")
        }

        columns += TableColumn<ClassData, Int>("Teacher ID").apply {
            cellValueFactory = PropertyValueFactory("teacherId")
        }
        ...
        setRowFactory {
            TableRow<ClassData>().apply {
                setOnMouseClicked { event ->
                    if (event.clickCount == 2) {
                        val selectedClass = item
                        popUp(selectedClass)
                    }
                }
            }
    ...
    fun loadClasses() {
        Thread {
            ...
            val request = getJson("$apiUrl/class/getAll", jwtToken = token)

            if (request != null) {
                val classList = json.decodeFromString<List<ClassData>>(request)
                Platform.runLater {
    ...
    fun createPopUp() {
        ...
        val teacherIdField = TextField().apply {
            promptText = "Teacher ID"
            style = "-fx-background-color: #3a3a3a; -fx-text-fill: white;"
        }
        ...
        val contentBox = VBox(5.0).apply {
            children.addAll(nameField, gradeIdField, teacherIdField, errorLabel)
        }
        ...
        createBtnNode.addEventFilter(ActionEvent.ACTION) { event ->
            ...
            val gradeId = gradeIdField.text.trim()
            val teacherId = teacherIdField.text.trim()

            if (name.isBlank() || gradeId.isBlank() || teacherId.isBlank()) {
            ...
            val request = ClassCreateRequest(
                name = name,
                gradeId = gradeId.toInt(),
                teacherId = teacherId.toInt()
            )

            Thread {
                val response = postJson("$apiUrl/class/create", request, jwtToken = token)
    ...
    fun popUp(classData: ClassData) {
        val alert = Alert(Alert.AlertType.NONE)
        alert.title = "Edit Class"
        alert.headerText = null
        ...
        val idLabel = Label("ID: ${classData.id}").apply {
            textFill = Color.web("#3a86ff")
            font = Font.font("Segoe UI", FontWeight.BOLD, 16.0)
            padding = Insets(0.0, 0.0, 10.0, 0.0)
        }

        val nameField = TextField(classData.name).apply {
            promptText = "Name"
            style = "-fx-background-color: #3a3a3a; -fx-text-fill: white;"
        }

        val gradeIdField = TextField(classData.gradeId.toString()).apply {
            promptText = "Grade ID"
            style = "-fx-background-color: #3a3a3a; -fx-text-fill: white;"
        }

        val teacherIdField = TextField(classData.teacherId.toString()).apply {
            promptText = "Teacher ID"
            style = "-fx-background-color: #3a3a3a; -fx-text-fill: white;"
        }
        ...
        updateBtnNode.addEventFilter(ActionEvent.ACTION) { event ->
            ...
            Thread {
                val response = postJson("$apiUrl/class/update", request, jwtToken = token)
        ...
        deleteBtnNode.addEventFilter(ActionEvent.ACTION) { event ->
            ...
            val request = ClassDeleteRequest(
                id = classData.id,
            )

            Thread {
                val response = postJson("$apiUrl/class/delete", request, token)
                Platform.runLater {
                    alert.close()
                    loadClasses()
                }
